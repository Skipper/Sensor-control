...
#define pGreen 2    // 27
#define pBlue 3     // 22
#define cantidadSegundos 139
#define sensorTemperaturaHumedad "AM2320" // Si7021  - AM2320
#define periodoParpadeo 23 // +1/24 Nos define cuantos segundos demorarà el parpadeo, desde que apagado-prendido-apagado
...
int main(){
    ...
    for (int i=0; i < segundos; i++){
        ...
        if ((temperature > 30) && (humidity > 60)) {
            ...
            clearColorInverted = ((unsigned char) clearColor - 255);
            clearColorInverted *= -1;
            //printf("clearColor %d \n",clearColor);
            //printf("clearColorInverted %d \n",clearColorInverted);
            setLCDBackLight(clearColorInverted, clearColorInverted, clearColorInverted);
            contador3060    = 0;
            contador30      = 0;
            contador60      = 0;
        }
        ...
        if (pruebaConstante == 1) {
            ...
            if (red > green && red > blue){
                ...
                printf("Dominant Color: Red - Green \n");
                setRGB(red, green, 0);
            } else if (red == blue){
                printf("Dominant Color: Red - Blue \n");
                setRGB(red, 0, blue);
            } else if (blue == green){
                printf("Dominant Color: Blue - Green \n");
                setRGB(0, green, blue);
            }
            printf("\n");
...
void FinDelProyecto(){
    ...
    int horas = 0,
        minutos = 0;
        segundos /= 2;
    horas = cantidadSegundos / (60*60);
    segundos %= 60*60;
    minutos = segundos / 60;
    segundos %= 60;

    char* horasChar     = (char*) std::to_string(horas).c_str();
    char* minutosChar   = (char*) std::to_string(minutos).c_str();
    char* segundosChar  = (char*) std::to_string(segundos).c_str();

    string tiempoTranscurrido = "";
    tiempoTranscurrido = horasChar;
    tiempoTranscurrido += "hor ";
    tiempoTranscurrido += minutosChar;
    tiempoTranscurrido += "min ";
    tiempoTranscurrido += segundosChar;
    ...
    delay(50);
    setLCDCursor(0, 1);
    setLCDText((char*) tiempoTranscurrido.c_str());
    delay (1900);
